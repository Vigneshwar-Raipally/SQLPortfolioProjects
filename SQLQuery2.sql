...
--The below query copies the duplicate data present in the actual table to temporary table
...
-- Query to delete the duplicate data
With RowNumCTE as(
Select *, Row_number() over (Partition by ParcelID, PropertyAddress, SaleDate, SalePrice, LegalReference order by uniqueid) row_num
from SQLPortfolioProject..HousingDataCleaning
)
...
--To check whether the duplicate data has been deleted or not use the below commented query
--With RowNumCTE as(
--Select *, Row_number() over (Partition by ParcelID, PropertyAddress, SaleDate, SalePrice, LegalReference order by uniqueid) row_num
--from SQLPortfolioProject..HousingDataCleaning
--)
...
--Delete some unwanted columns from the table
